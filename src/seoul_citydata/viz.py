...
def _population_insights(df: pd.DataFrame) -> dict[str, Any]:
    """주요 장소의 거주비율과 가장 가까운 인구 예보를 대시보드용으로 집계."""
    records = df.to_dict("records")
    weighted_resident = 0.0
    weighted_non_resident = 0.0
    weight_total = 0.0
    forecasts: list[dict[str, Any]] = []
    for row in records:
        raw_population = row.get("ppltn_mid")
        population = (
            float(raw_population)
            if raw_population is not None and pd.notna(raw_population)
            else 0.0
        )
        resident = row.get("resident_rate")
        non_resident = row.get("non_resident_rate")
        if (
            population
            and resident is not None
            and non_resident is not None
            and pd.notna(resident)
            and pd.notna(non_resident)
        ):
            weighted_resident += float(resident) * population
            weighted_non_resident += float(non_resident) * population
            weight_total += population

        area_forecasts = row.get("forecast")
        if not isinstance(area_forecasts, list) or not area_forecasts:
            continue
        nearest = area_forecasts[0]
        predicted_min = nearest.get("fcst_ppltn_min")
        predicted_max = nearest.get("fcst_ppltn_max")
        if (
            predicted_min is None
            or predicted_max is None
            or pd.isna(predicted_min)
            or pd.isna(predicted_max)
        ):
            continue
        predicted = (float(predicted_min) + float(predicted_max)) / 2
        delta_rate = ((predicted - population) / population * 100) if population else None
        forecasts.append(
            {
                "area": row.get("area"),
                "current_population": round(population),
                "forecast_time": nearest.get("fcst_time"),
                "forecast_population": round(predicted),
                "delta_rate": round(delta_rate, 1) if delta_rate is not None else None,
                "congest_level": nearest.get("fcst_congest_level"),
            }
        )

    forecasts.sort(key=lambda item: item["current_population"], reverse=True)
    return {
        "resident_rate": round(weighted_resident / weight_total, 1)
        if weight_total
        else None,
        "non_resident_rate": round(weighted_non_resident / weight_total, 1)
        if weight_total
        else None,
...
def build_dashboard_data(
    ...
    return {
        ...
        "corr": corr_data,
        "population_insights": _population_insights(df),
...
def write_pages_dashboard(
    ...
    movement_data: dict[str, Any] | None = None,
) -> tuple[str, str]:

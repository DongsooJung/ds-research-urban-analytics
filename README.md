...
- **외부 유입 생활인구** (`movement.py`): 대도시권 생활인구 최신 일별 원본을 하루 한 번
  처리해 08시·18시 서울 외 거주 생활인구의 출발 지역과 체류 행정동을 비교
...
서울 열린데이터광장 「서울시 대도시권 생활인구(행정동별)」 (`se250mSpopOrgnCt`)

```
https://data.seoul.go.kr/dataList/OA-22850/A/1/datasetView.do
```

대도시권 자료는 최신 일별 ZIP을 기준일당 한 번 집계하며, 3명 이하 비식별 값은 합계에서 제외된다.
...
│   ├── movement.py     # 수도권·전국 외부 유입 생활인구 집계
...
| 외부 유입 | 08시·18시 서울 외 거주 생활인구, 거주지·체류 행정동·자치구 |
